---
title: <center>ST 53 SAGD Data</center>
...
`r kable(df_project,digits=c(0,0,2,0,0),col.names=c("Project","Operator","Cumulative Steam-to-Oil Ratio",paste("Bitumen Production (Barrels Per Day)",max(df1$Date),sep=" "),"Recovery Method"))%>%kable_styling()`
...
`r df_year%>%kable(digits=c(0,0,0,2,2),col.names=c("Year","CSS","SAGD","CSS","SAGD"))%>%kable_styling()%>%add_header_above(c(" " = 1, "Bitumen Production" = 2, "Steam-to-oil Ratio" = 2))%>%add_header_above(c(" " = 1, "Annual, Production-Weighted Average Performance Data" = 4))`

   
  
```{r graphs, cache=FALSE}
...
```{r SOR_cumul, cache=FALSE,fig.width=12, fig.height=8}
#create a data frame that is only SAGD and CSS
...
`r kable(in_situ_nir%>%select(-c(1,2,6)),digits=c(0,1,0,3,2),col.names=c("Year","GHGs (Mt)","Bitumen Production (Barrels Per Day)","GHG per Barrel","Average Steam-Oil Ratio"))%>%kable_styling()`
