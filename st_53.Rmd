...
fix_names<-function(data_sent){
  data_sent<-data_sent %>% mutate(`Scheme Name`= case_when(
    #Christina Lake
    grepl("MEG", Operator) & grepl("Christina", `Scheme Name`)  ~ "Christina Lake North (ex-MEG)",
    grepl("Meg", Operator) & grepl("Christina", `Scheme Name`)  ~ "Christina Lake North (ex-MEG)",
    grepl("10773", `Approval Number`)  ~ "Christina Lake North (ex-MEG)",
    ...
    grepl("Tucker", `Scheme Name`)  ~ "Tucker",
    TRUE ~ `Scheme Name`
  )
  )
...
st_53_online<-function(download=F){
  ...
  years<-seq(2000,year(Sys.Date() %m-% months(3))) #2 month data lag
  ...
  for(year in years){
    ...
    if(year==2025){
      filename<-paste("st53_",year,".xls",sep="")
      address<-paste("https://www.aer.ca/prd/documents/sts/ST53/ST53_2025.xls",sep="")
    }
    if(year==2026){
      filename<-paste("st53_",year,".xls",sep="")
    ...
    #https://www.aer.ca/documents/sts/ST53/ST53_Current.xls
    if(download==T & year==2026)
     if(file.exists(filename))
       file.remove(filename)
    if(download==T)
      if(!file.exists(filename))
        download_forbidden_file(address,filename)
    ...
    bitumen_data<-bitumen_data %>% select(-`Monthly Average`)%>%
      pivot_longer(-c(1:5),names_to = "Month",values_to = "Bitumen")%>%
      mutate(Month=match(as.character(Month),month.abb))%>%
      filter(!is.na(Bitumen))
    ...
    steam_data<-steam_data %>% select(-`Monthly Average`)%>%
      pivot_longer(-c(1:5),names_to = "Month",values_to = "Steam")%>%
      mutate(Month=match(as.character(Month),month.abb))%>%
      filter(!is.na(Steam))
    ...
    water_data<-water_data %>% select(-`Monthly Average`)%>%
      pivot_longer(-c(1:5),names_to = "Month",values_to = "water")%>%
      mutate(Month=match(as.character(Month),month.abb))%>%
      filter(!is.na(water))
...
#merge them all  
    ...
    bitumen_data<-bitumen_data %>% group_by(Operator,`Scheme Name`,Area,`Recovery Method`,`Approval Number`,Month,Date,year) %>%
...
save(ST53_data,file="st53_all.rdata")
...
summarise(CSOR=sum(steam,na.rm = T)/sum(bitumen,na.rm = T),
  ...
  mutate(Bitumen=format(round(Bitumen/1000,0), big.mark = ","))%>%
  arrange(`Scheme Name`)

#  arrange(-Bitumen)
  

df_year<-df1 %>% group_by(year,`Recovery Method`) %>% summarise(Bitumen=mean(Bitumen)*6.2929/1000,SOR=mean(SOR))%>% ungroup()%>%
...
`r kable(df_project,digits=c(0,0,2,2,0),align=c("l","l","c","c","c"),col.names=c("Project","Operator","Cumulative Steam-to-Oil Ratio",paste("Bitumen Production (1000 Barrels Per Day),\nmonth ended",format(max(df1$Date),"%B, %Y"),sep=" "),"Recovery Method"))%>%kable_styling()`
...
`r df_year%>%kable(digits=c(0,0,0,0,2,2),align=c("l","c","c","c","c","c"),col.names=c("Year","Commercial CSS","Commercial SAGD","Commercial Total","CSS","SAGD"))%>%kable_styling()%>%add_header_above(c(" " = 1, "Bitumen Production (1000 Barrels per Day)" = 3, "Steam-to-oil Ratio" = 2))%>%add_header_above(c(" " = 1, "Annual, Production-Weighted Average Performance Data" = 5))`
...
prod_graph<-
  ggplot(df1,aes(Date,Bitumen*6.2929/10^3,group=`Recovery Method`,colour=`Recovery Method`,fill=`Recovery Method`)) +
  ...
  theme_irpp()+
  expand_limits(y=0)+
  labs(y="Bitumen production (1000s of barrels per day)",x=NULL,
...
water_graph<-ggplot(df1,aes(Date,water/Bitumen,group=`Recovery Method`,colour=`Recovery Method`,fill=`Recovery Method`)) +
  ...
  scale_y_continuous(breaks=pretty_breaks(),expand=c(0,0))+
  theme_irpp()+
  labs(y="Water production ratio (barrels per barrel bitumen)",x=NULL,
...
sor_graph<-ggplot(df1,aes(Date,SOR,group=`Recovery Method`,colour=`Recovery Method`)) +
  ...
  scale_y_continuous(breaks=pretty_breaks(),expand=c(0,0))+  
  theme_irpp()+
  labs(y="Steam:bitumen ratio",x=NULL,
...
p1<-ggplot(filter(df2),aes(Date,cumul_sor,group=`Recovery Method`,colour=`Recovery Method`)) +
  ...
  scale_y_continuous(breaks=pretty_breaks(4),expand=c(0,0))+
  expand_limits(y=c(0,6))+
  theme_irpp()+
  labs(y="Cumulative steam:bitumen ratio",x=NULL,
       title=paste("Cumulative steam:oil Ratios",sep=""),
       caption="Source: AER/ERCB ST53 data, graph by Andrew Leach.")
p2<-ggplot(filter(df2),aes(Date,Steam/Bitumen,group=`Recovery Method`,colour=`Recovery Method`)) +
  ...
  expand_limits(y=c(0,6))+
  theme_irpp()+
  labs(y="Monthly steam:bitumen ratio",x=NULL,
       title=paste("Steam:oil Ratios by Recovery Method",sep=""),
       caption="Source: AER/ERCB ST53 data, graph by Andrew Leach.")
...
date_span <- diff(range(scheme_data$Date, na.rm = TRUE))

date_breaks <- if (date_span < lubridate::years(2)) {
  "1 month"
} else {
  "2 years"
}
...
plot_a<-
  ...
  labs(y="Steam Production (1000 barrels per day)",x=NULL,
       title=paste(scheme_name,"Steam Production"))+
  theme_irpp(base_size = 12)+
  expand_limits(y=0)
plot_b<-ggplot(scheme_data)+
  ...
  labs(y="Bitumen Production (1000 barrels per day)",x=NULL,
...
plot_c<-ggplot(scheme_data)+
  ...
  scale_y_continuous(expand = c(0,0),breaks = pretty_breaks())+
  #geom_hline(yintercept = 0) + 
  labs(y="Steam-to-oil ratio (barrels per barrel)",x=NULL,
       title=paste(scheme_name,"Monthly Steam-to-oil Ratio"),
       subtitle="Monthly data capped at SOR of 10",     
       caption="Dashed line is province-wide monthly average for recovery method (SAGD or CSS)")+
  theme_irpp(base_size = 12)+
  expand_limits(y=0)
plot_d<-ggplot(scheme_data)+
  ...
  scale_y_continuous(expand = c(0,0),breaks = pretty_breaks())+
  #geom_hline(yintercept = 0) + 
  labs(y="Cumulative steam-to-oil ratio (barrels per barrel)",x=NULL,
...
### Christina Lake North (CVE, ex-MEG)

```{r fig.width=12, fig.height=8, scheme_plot_1, cache=FALSE}

sagd_plot("Christina Lake North (ex-MEG)")
...
```{r fig.width=12, fig.height=8, scheme_plot_narrows, cache=FALSE}

sagd_plot("Narrows Lake")
...
```{r fig.width=12, fig.height=8, scheme_plot_leismer, cache=FALSE}
sagd_plot("Leismer")
```
